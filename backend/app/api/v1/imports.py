...
from app.schemas.import_job import ImportJobResponse, ImportJobListResponse, ImportItemResponse, ImportItemUpdateRequest, ImportConfirmRequest
...
@router.get("/{id}/items", response_model=list[ImportItemResponse])
async def get_import_items(id: UUID, db: DBSession):
    return await repo.get_items_by_job(db, id)
...
async def update_import_item(id: UUID, item_id: UUID, data: ImportItemUpdateRequest, db: DBSession):
    item = await repo.update_item(db, item_id, data.model_dump(exclude_unset=True))
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    return item

@router.post("/{id}/confirm", response_model=ImportJobResponse)
async def confirm_import(id: UUID, data: ImportConfirmRequest, db: DBSession, storage: Storage):
    job = await service.confirm_import(db, id, data.approved_item_ids, data.rejected_item_ids, storage)
    if not job:
        raise HTTPException(status_code=404, detail="Import not found")
    return job
